// Exercicio feito com participacoes dos colegas: Lineida Simoes e Rosario Mutamira
...
void gerar_estados_seguintes(Puzzle *puzzle, Puzzle *estados_seguintes[4]) {
    ...
    // Encontrar a posicao da peca vazia (0)
    ...
    for (i = 0; i < 4; i++) {
        ...
        // Verificar se o novo estado eh valido
        if (nova_linha >= 0 && nova_linha < TAMANHO && nova_coluna >= 0 && nova_coluna < TAMANHO) {
            ...
            // Trocar a posicao da peca vazia com a peca adjacente
...
int main() {
    ...
    Puzzle estado_inicial = {
        {{1, 2, 3}, {6, 5, 4}, {0, 7, 8}}  // 0 representa a posicao vazia
    };
    ...
    if (frente >= tras) {
        printf("Puzzle nao pode ser resolvido.\n");
    }
